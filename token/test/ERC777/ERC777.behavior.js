  ...
  describe('direct send', function () {
    ...
    context('when the sender has no tokens', async function () {
      before(async function() {
        const balance = await this.token.balanceOf(holder);
        await this.token.send(this.bufferAccount, balance, data, { from: holder });
      });
      after(async function() {
        const balance = await this.token.balanceOf(this.bufferAccount);
        await this.token.send(holder, balance, data, { from: this.bufferAccount });
      });
...
function shouldBehaveLikeERC777UnauthorizedOperatorSend (holder, recipient, operator, data, operatorData) {
  describe('operator send', function () {
    it('reverts', async function () {
      await expectRevert.unspecified(this.token.operatorSend(holder, recipient, new BN('0'), data, operatorData, {from: operator}));
    });
  });
}
...
function shouldBehaveLikeERC777UnauthorizedOperatorBurn (holder, operator, data, operatorData) {
  describe('operator burn', function () {
    it('reverts', async function () {
      await expectRevert.unspecified(this.token.operatorBurn(holder, new BN('0'), data, operatorData, {from: operator}));
    });
  });
}
...
function shouldBehaveLikeERC777InternalMint (recipient, operator, amount, data, operatorData) {
  ...
  it('reverts when minting tokens for the zero address', async function () {
    await expectRevert.unspecified(
      this.token.mint(ZERO_ADDRESS, amount, data, operatorData, { from: operator }),
    );
  });
}
...
function shouldInternalMintTokens (operator, to, amount, data, operatorData) {
  it(`can (internal) mint an amount of ${amount}`, async function () {
    ...
    const { logs } = await this.token.mint(to, amount, data, operatorData, true, { from: operator });
...
function shouldBehaveLikeERC777SendBurnMintInternalWithReceiveHook (contractOwner, operator, amount, data, operatorData) {
  context('when TokensRecipient reverts', function () {
    ...
    it('mint (internal) reverts', async function () {
      await expectRevert.unspecified(
        this.token.mint(this.recipient, amount, data, operatorData, true, { from: operator }),
      );
    });
  });
  ...
  context('when TokensRecipient does not revert', function () {
    ...
    it('TokensRecipient receives mint (internal) data and is called after state mutation', async function () {
      ...
      const { tx } = await this.token.mint(
        this.recipient, amount, data, operatorData, true, { from: contractOwner }
      );
...
function shouldBehaveLikeERC777SendBurnWithSendHook (contractOwner, operator, amount, data, operatorData) {
  context('when TokensSender reverts', function () {
    ...
    it('burn reverts', async function () {
      await expectRevert.unspecified(burnFromHolder(contractOwner, this.token, this.sender, amount, data));
    });
...
async function burnFromHolder(contractOwner, token, holder, amount, data) {
  if ((await web3.eth.getCode(holder)).length <= '0x'.length) {
    return token.burn(amount, data, { from: contractOwner });
  } 
  else {
    // assume holder is ERC777SenderRecipientMock contract
    return (await ERC777SenderRecipientMock.at(holder)).burn(token.address, amount, data, {from: contractOwner});
  }
}
